...
public class CircleEncoder extends DrawEncoder {
    ...
    private List<List<LandmarkInfo>> trackedObjects = new ArrayList<>();
    ...
    public synchronized void draw(final Canvas canvas) {
        ...
        for (int i = 0; i < trackedObjects.size(); i++) {
            for (int j = 0; j < trackedObjects.get(i).size() ; j++) {
                float x = 0;
                float y = 0;
                x = trackedObjects.get(i).get(j).getX();
                y = trackedObjects.get(i).get(j).getY();
    ...
    public synchronized void processResults(Object results) {
        ...
        if (results.getClass() == ArrayList.class) {
            trackedObjects = (List<List<LandmarkInfo>>) results;
        }
    }
